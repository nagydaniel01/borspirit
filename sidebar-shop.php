<!--
<div id="toolbar-product" class="toolbar toolbar--products">
    <input type="search" name="keyword" id="keyword" class="form-control search js-search" placeholder="<?php //echo esc_html__('Search...', 'borspirit'); ?>" >
</div>

<div class="filters filters--products">
    <div class="filters__header">
        <h2 class="filters__title"><?php //echo esc_html__('Filter', 'borspirit'); ?></h2>
    </div>

    <div id="filter-list-vertical" class="page__filters-inner js-filter-list">
        <?php //get_template_part('template-parts/queries/query', 'product-attributes'); ?>
    </div>

    <div class="filters__footer">
        <a class="button button--reset js-filters-reset"><?php //echo esc_html__('Reset', 'borspirit'); ?></a>
    </div>
</div>

<div class="content content--products">
    <div id="product-list" class="list list--products" data-posts-per-page="4">
        <?php //get_template_part( 'template-parts/queries/query', 'product' ); ?>
    </div>
</div>
-->

<!-- https://www.youtube.com/watch?v=gOJ4zXuNyQ8 -->

<nav class="section__navigation navbar navbar-expand-lg">
    <button class="navbar-toggler" type="button" data-bs-toggle="offcanvas" data-bs-target="#offcanvasProductFilter" aria-controls="offcanvasProductFilter">
        <?php echo esc_html__( 'Filter', 'borspirit' ); ?>
        <svg class="icon icon-filter">
            <use xlink:href="#icon-filter"></use>
        </svg>
    </button>
    <div class="offcanvas offcanvas-start" id="offcanvasProductFilter" tabindex="-1" aria-labelledby="offcanvasProductFilterLabel">
        <div class="offcanvas-header">
            <h5 class="offcanvas-title" id="offcanvasProductFilterLabel">
                <?php echo esc_html__( 'Filter', 'borspirit' ); ?>
            </h5>
            <button type="button" class="btn-close" data-bs-dismiss="offcanvas" aria-label="<?php echo esc_attr('Close', 'borspirit'); ?>"></button>
        </div>
        <div class="offcanvas-body">
            <?php echo do_shortcode( '[yith_wcan_filters slug="default-preset"]' ); ?>
        </div>
    </div>
</nav>